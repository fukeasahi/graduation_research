...
# 参考にしたGitHUB
# https://www.youtube.com/watch?v=cEAHzsFUtok
# https://github.com/moshesipper/tiny_gp/blob/master/tiny_gp.py
...
def generate_dataset(): # generate 101 data points from target_func
    ...
    for i in range(len(np.array(df["平均気温(℃)"]))):
      dataset.append([np.array(df["平均気温(℃)"][i]), np.array(df["数量(t)"][i] * 1000 * df["円/kg"][i]) ])
    return dataset

# def generate_dataset():
#   df = pd.read_excel("/content/12401905810_冨家旭陽.xlsx", sheet_name = "date2")
#   dataset = df["平均気温(℃)"]
#   return dataset
